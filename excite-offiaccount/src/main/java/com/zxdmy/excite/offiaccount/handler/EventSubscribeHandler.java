...
public class EventSubscribeHandler extends AbstractHandler {
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, Map<String, Object> context, WxMpService wxMpService, WxSessionManager sessionManager) throws WxErrorException {
        ...
        if (wxMessage.getEventKey() == null || wxMessage.getEventKey().equals("")) {
            ...
            // 获取关注后返回的内容
            mpReply = mpReplyService.getOneReplyByTypeOrKey(OffiaccountConsts.ReplyType.SUBSCRIBE_REPLY, null);
        }
        // 通过带参数的二维码关注：场景值不为空，需要根据场景值进行二次处理（比如登录）
            ...
            // 获取登录成功后返回的内容
            mpReply = mpReplyService.getOneReplyByTypeOrKey(OffiaccountConsts.ReplyType.SCAN_LOGIN_REPLY, null);
        }
        // 构造返回的消息
