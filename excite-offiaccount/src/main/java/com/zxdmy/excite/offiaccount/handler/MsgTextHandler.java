...
public class MsgTextHandler extends AbstractHandler {
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, Map<String, Object> context, WxMpService wxMpService, WxSessionManager sessionManager) throws WxErrorException {
        ...
        if (content.length() == 5 && (content.charAt(0) == 'L' || content.charAt(0) == 'l')) {
            ...
            // 获取登录成功后返回的内容
            msgReply = mpReplyService.getOneReplyByTypeOrKey(OffiaccountConsts.ReplyType.SCAN_LOGIN_REPLY, null);
        }
        // 否则，是普通消息
        else {
            // 从[关键词]数据库检索需要回复的信息（如果有多条，选择最近的一条）
            msgReply = mpReplyService.getOneReplyByTypeOrKey(OffiaccountConsts.ReplyType.KEYWORD_REPLY, content);
            // 如果[关键词]为空，则返回默认的信息
            if (null == msgReply) {
                msgReply = mpReplyService.getOneReplyByTypeOrKey(OffiaccountConsts.ReplyType.DEFAULT_REPLY, null);
            }
        }
        // 根据回复详情，构造消息
